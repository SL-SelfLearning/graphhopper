...
public class GraphHopper implements GraphHopperAPI {
    ...
    public GraphHopper init(CmdArgs args) {
        ...
        if (!flagEncodersStr.isEmpty()) {
            if (bytesForFlags > 8) {
                ...
                setEncodingManager(new EncodingManager.Builder(new TagsParserOSM(), bytesForFlags).addGlobalEncodedValues().addAll(flagEncoderFactory, flagEncodersStr, bytesForFlags).build());
            }
        }
    ...
    public Weighting createWeighting(HintsMap hintsMap, FlagEncoder encoder, Graph graph) {
        ...
        if (hintsMap.has(Routing.BLOCK_AREA)) {
            ...
            GraphEdgeIdFinder.BlockArea blockArea = new GraphEdgeIdFinder(graph, locationIndex).
                    parseBlockArea(blockAreaStr, new DefaultEdgeFilter(encoder, encoder.getPrefix()));
    ...
    public List<Path> calcPaths(GHRequest request, GHResponse ghRsp) {
        ...
        try {
            ...
            EdgeFilter edgeFilter = vehicle.equals(EncodingManager.ENCODER_NAME) ? EdgeFilter.ALL_EDGES : new DefaultEdgeFilter(encoder, encoder.getPrefix());
