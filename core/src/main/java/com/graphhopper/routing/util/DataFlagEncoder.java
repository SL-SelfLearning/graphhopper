...
public class DataFlagEncoder extends AbstractFlagEncoder {
    ...
    public Map<String, TagParser> createTagParsers(final String prefix) {
        ...
        map.put(prefix + "average_speed", new TagParser() {
            @Override
            public String getName() {
                return prefix + "average_speed";
            }
        ...
        if (isStoreHeight())
            map.put("max_height", TagParserFactory.Truck.createMaxHeight(new DecimalEncodedValue("max_height", 7, 0, 0.1, false),
                    osmWayFilter));
        // value range: [1.0t, 59.5t]
        if (isStoreWeight())
            map.put("max_weight", TagParserFactory.Truck.createMaxWeight(new DecimalEncodedValue("max_weight", 10, 0, 0.1, false),
                    osmWayFilter));
        // value range: [2.5m, 3.5m]
        if (isStoreWidth())
            map.put("max_width", TagParserFactory.Truck.createMaxWidth(new DecimalEncodedValue("max_width", 6, 0, 0.1, false),
                    osmWayFilter));
