...
public class DataFlagEncoderTest {
    ...
    public DataFlagEncoderTest() {
        ...
        roadClassEnc = encodingManager.getStringEncodedValue("road_class");
        maxSpeedEnc = encodingManager.getDecimalEncodedValue(encoder.getPrefix() + "max_speed");
        surfaceEnc = encodingManager.getStringEncodedValue("surface");
    ...
    public void stringToMeter() {
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("1.5"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("1.5m"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("1.5 m"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("1.5   m"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("1.5 meter"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("4 ft 11 in"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToMeter("4'11''"), DELTA);


        assertEquals(3, TagParserFactory.Truck.stringToMeter("3 m."), DELTA);
        assertEquals(3, TagParserFactory.Truck.stringToMeter("3meters"), DELTA);
        assertEquals(0.8 * 3, TagParserFactory.Truck.stringToMeter("~3"), DELTA);
        assertEquals(3 * 0.8, TagParserFactory.Truck.stringToMeter("3 m approx"), DELTA);

        // 2.743 + 0.178
        assertEquals(2.921, TagParserFactory.Truck.stringToMeter("9 ft 7in"), DELTA);
        assertEquals(2.921, TagParserFactory.Truck.stringToMeter("9'7\""), DELTA);
        assertEquals(2.921, TagParserFactory.Truck.stringToMeter("9'7''"), DELTA);
        assertEquals(2.921, TagParserFactory.Truck.stringToMeter("9' 7\""), DELTA);

        assertEquals(2.743, TagParserFactory.Truck.stringToMeter("9'"), DELTA);
        assertEquals(2.743, TagParserFactory.Truck.stringToMeter("9 feet"), DELTA);
    }

    @Test(expected = NumberFormatException.class)
    public void stringToMeterException() {
        // Unexpected values
        TagParserFactory.Truck.stringToMeter("height limit 1.5m");
    }

    @Test
    public void stringToTons() {
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("1.5"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("1.5 t"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("1.5   t"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("1.5 tons"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("1.5 ton"), DELTA);
        assertEquals(1.5, TagParserFactory.Truck.stringToTons("3306.9 lbs"), DELTA);
        assertEquals(3, TagParserFactory.Truck.stringToTons("3 T"), DELTA);
        assertEquals(3, TagParserFactory.Truck.stringToTons("3ton"), DELTA);

        // maximum gross weight
        assertEquals(6, TagParserFactory.Truck.stringToTons("6t mgw"), DELTA);
    }

    @Test(expected = NumberFormatException.class)
    public void stringToTonsException() {
        // Unexpected values
        TagParserFactory.Truck.stringToTons("weight limit 1.5t");
    }
    ...
    public void testSpatialId() {
        ...
        EdgeIteratorState e1 = GHUtility.createEdge(graph, accessEnc, 0, 1, 1, true);
        EdgeIteratorState e2 = GHUtility.createEdge(graph, accessEnc, 0, 2, 1, true);
        EdgeIteratorState e3 = GHUtility.createEdge(graph, accessEnc, 0, 3, 1, true);
        EdgeIteratorState e4 = GHUtility.createEdge(graph, accessEnc, 0, 4, 1, true);
