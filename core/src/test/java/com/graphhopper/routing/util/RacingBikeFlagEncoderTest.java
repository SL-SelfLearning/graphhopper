...
public class RacingBikeFlagEncoderTest extends AbstractBikeFlagEncoderTester {
    ...
    protected BikeCommonFlagEncoder createBikeEncoder() {
        encodingManager = new EncodingManager.Builder().addAllFlagEncoders("bike,racingbike").build();
    ...
    public void testSacScale() {
        ...
        // This looks like a tagging error:
        way.setTag("highway", "cycleway");
        way.setTag("sac_scale", "mountain_hiking");
        // we are conservative and disallow this
        assertTrue(encoder.getAccess(way).canSkip());
    }
    ...
    public void testHandleWayTagsInfluencedByRelation() {
        ...
        IntsRef flags = encoder.handleWayTags(encodingManager.createIntsRef(), osmWay, EncodingManager.Access.WAY, relFlags);
        ...
        flags = encoder.handleWayTags(encodingManager.createIntsRef(), osmWay, EncodingManager.Access.WAY, relFlags);
        assertEquals(2, encoder.getSpeed(flags), 1e-1);
        assertPriority(AVOID_AT_ALL_COSTS.getValue(), osmWay, relFlags);
    ...
    public void testAvoidanceOfHighMaxSpeed() {
        ...
        assertEquals(20, encoder.getSpeed(encoder.setSpeed(encodingManager.createIntsRef(), encoder.applyMaxSpeed(osmWay, 20))), 1e-1);
        assertPriority(PREFER.getValue(), osmWay);
