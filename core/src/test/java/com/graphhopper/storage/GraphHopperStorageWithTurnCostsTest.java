...
public class GraphHopperStorageWithTurnCostsTest extends GraphHopperStorageTest {
    ...
    public void testSave_and_fileFormat() throws IOException {
        ...
        GHUtility.createEdge(graph,carAccessEnc,3, 4, 123, true).setWayGeometry(Helper.createPointList3D(4.4, 5.5, 0, 6.6, 7.7, 0));
        checkGraph(graph);
    }
    ...
    public void testEnsureCapacity() throws IOException {
        ...
        for (int nodeId = 51; nodeId < 100; nodeId++) {
            GHUtility.createEdge(graph,carAccessEnc,50, nodeId, r.nextDouble(), true);
        }
        for (int nodeId = 0; nodeId < 50; nodeId++) {
            GHUtility.createEdge(graph,carAccessEnc,nodeId, 50, r.nextDouble(), true);
        }
