...
public class OSMReaderTest {
    ...
    public void testMaxSpeed() {
        ...
        iter.next();
        assertEquals(35, carEncoder.getSpeed(iter.getData()), 1e-1);
    }
    ...
    public void testRoadAttributes() {
        ...
        EncodingManager em = new EncodingManager.Builder().addAll(Arrays.asList(encoder), 8).build();
        ...
        DecimalEncodedValue heightEnc = em.getDecimalEncodedValue(TagParserFactory.Truck.MAX_HEIGHT);
        DecimalEncodedValue widthEnc = em.getDecimalEncodedValue(TagParserFactory.Truck.MAX_WIDTH);
        DecimalEncodedValue weightEnc = em.getDecimalEncodedValue(TagParserFactory.Truck.MAX_WEIGHT);
    ...
    public void testEstimatedCenter() {
        final CarFlagEncoder encoder = new CarFlagEncoder() {
            private EncodedValue08 objectEncoder;

            @Override
            public int defineNodeBits(int index, int shift) {
                shift = super.defineNodeBits(index, shift);
                objectEncoder = new EncodedValue08("oEnc", shift, 2, 1, 0, 3, true);
                return shift + 2;
            }
...
//        }
    ...
    class GraphHopperFacade extends GraphHopperOSM {
        ...
        public GraphHopperFacade(String osmFile, boolean turnCosts) {
            ...
            carAccessEnc = getEncodingManager().getBooleanEncodedValue(TagParserFactory.Foot.ACCESS);
        }
